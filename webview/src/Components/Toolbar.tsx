    ...
    <header className="toolbar">
      <div className="tabs">
        {APP_TABS.map((tab) => {
          ...
          return (
            <button key={tab.key} className={`tabButton ${props.activeTab === tab.key ? "isActive" : ""}`} type="button" disabled={isActionInProgress} onClick={() => props.onSelectTab(tab.key)}>
              {tab.label}
              {isTabLoading ? <span className="tabLoadingSpinner"><Spinner /></span> : null}
              {!isTabLoading && count > 0 ? <span className={tab.key === "vulnerabilities" ? "warningTabBadge" : ""}>{tab.key === "vulnerabilities" ? `! ${count}` : count}</span> : null}
            </button>
          );
        })}
        <button className="iconButton refreshButton" type="button" disabled={isActionInProgress} title="Refresh" onClick={props.onRefresh}>{props.actionBusy === "refresh" ? <Spinner /> : null}</button>
        <button className={`iconButton infoButton ${props.infoOpen ? "isActive" : ""}`} type="button" disabled={isActionInProgress} title="Info" onClick={props.onToggleInfo}>i</button>
        <button className={`iconButton gearButton ${props.settingsOpen ? "isActive" : ""}`} type="button" disabled={isActionInProgress} title="Settings" onClick={props.onToggleSettings}>Settings{disabledSourceProblemCount > 0 ? <span className="toolbarWarningBadge" aria-label="Disabled source problems">!</span> : null}{sourceProblemCount > 0 ? <span className="toolbarAlertBadge">{sourceProblemCount}</span> : null}</button>
      </div>
      <div className="filters">
        <div className="searchBox">
          <input className="searchInput" value={props.searchTerm} disabled={isActionInProgress} placeholder="Search packages..." data-input="search" onChange={(event) => props.onChangeSearch(event.target.value)} />
          {props.searchTerm ? <button className="clearSearchButton" type="button" disabled={isActionInProgress} title="Clear search" onClick={() => props.onChangeSearch("")}>x</button> : null}
        </div>
        <label className="checkboxLabel">
          <input checked={props.includePrerelease} disabled={isActionInProgress} type="checkbox" onChange={(event) => props.onChangePrerelease(event.target.checked)} />
          Prerelease
        </label>
        <span className="toolbarSpacer" />
        <select className="sourceSelect toolbarSourceSelect" aria-label="NuGet source" disabled={isActionInProgress} value={props.selectedSourceName || ALL_SOURCES} onChange={(event) => props.onChangeSource(event.target.value)}>
        ...
        {(props.activeTab === "updates" || props.activeTab === "consolidated") ? (
          <div className="toolbarBulkActions">
            <button className="primaryButton" type="button" disabled={selectedPackageCount === 0 || isActionInProgress} onClick={props.onBulkAction}>
              {props.actionBusy === "bulk" ? <Spinner /> : null} {props.activeTab === "updates" ? "Update" : "Consolidate"}
            </button>
            <button className="secondaryButton" type="button" disabled={props.visibleGroups.length === 0 || isActionInProgress} onClick={props.onToggleVisibleBulkPackages}>
