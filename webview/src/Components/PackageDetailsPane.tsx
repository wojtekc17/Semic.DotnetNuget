...
export function PackageDetailsPane(props: {
    ...
    return (
        <section className="packagePreview" data-testid="package-details-pane">
            ...
            <ProjectVersionTable activeTab={props.activeTab} packageInfo={props.packageInfo} projects={props.projects} selectedProjectIds={props.selectedProjectIds} selectedVersion={props.selectedPackageVersion} onToggleAllProjects={props.onToggleAllProjects} onToggleProject={props.onToggleProject} />
...
function ActionForm(props: {
    ...
    const versionSelect = (
        <select className="sourceSelect" data-testid="package-version-select" value={props.selectedVersion} onChange={(event) => props.onChangeVersion(event.target.value)}>
            {props.versions.map((version) => <option key={version} value={version}>{version}</option>)}
        </select>
    );
    const installButton = (label: string) => (
        <button className="primaryButton" data-testid="package-install-action" disabled={props.selectedProjectCount === 0 || props.installBusy} type="button" onClick={() => props.onInstall(props.selectedVersion)}>
    ...
    const uninstallButton = props.activeTab === "browse"
        ? null
        : (
            <button className="dangerButton" disabled={props.selectedProjectCount === 0 || props.uninstallBusy} type="button" onClick={props.onUninstall}>
...
function ProjectVersionTable(props: {
    ...
    return (
        <div className="versionsPanel">
            <div className="projectSelectHeader"><strong>Projects</strong><button className="linkButton" type="button" onClick={props.onToggleAllProjects}>Toggle all</button></div>
                ...
                <div className="projectVersionScroller">
                    {rows.map((row) => (
                        <label key={row.projectId} className="projectVersionRow">
                            <input checked={props.selectedProjectIds.has(row.projectId)} type="checkbox" onChange={(event) => props.onToggleProject(row.projectId, event.target.checked)} />
